...
export class AddFailurereson1788352718153 implements MigrationInterface {
  name = 'AddFailurereson1788352718153';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_4eb325684ffcdee301eeebfd19" ON "guardians" ("guardian_user_id") `,
    );
    await queryRunner.query(
      `ALTER TABLE "guardians" ADD CONSTRAINT "FK_4eb325684ffcdee301eeebfd19a" FOREIGN KEY ("guardian_user_id") REFERENCES "users"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "guardians" DROP CONSTRAINT "FK_4eb325684ffcdee301eeebfd19a"`,
    );
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX "IDX_guardians_wallet_email_open" ON "guardians" ("wallet_id", "guardian_email") WHERE ((status = ANY (ARRAY['invited'::guardians_status_enum, 'active'::guardians_status_enum])) AND (deleted_at IS NULL))`,
    );
    await queryRunner.query(
