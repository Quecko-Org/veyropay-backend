...
export class TurnkeyService {
  ...
  async provisionSubOrganization(
    ...
    try {
      ...
      throw new ProviderException(
        TURNKEY_PROVIDER_NAME,
        `Unable to create Turnkey sub-organization ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async findSubOrganizationByOidcToken(oidcToken: string): Promise<string | null> {
    try {
      ...
      throw new ProviderException(
        TURNKEY_PROVIDER_NAME,
        `Unable to look up Turnkey sub-organization${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
